...
that is, $\Sub{F} : a \rightarrow \Sub{F} a$.
...
\rightarrow \Set{F}$.
$\;\Div{F} : a \rightarrow \One{F}\Div{F} a$.
...
$\Zero{F}() \,\Add{F}\, a \,=\, a$.
Typically the nullary function $\Zero{F}()$ is identified with its
value as an element of $\Set{F}$: $\Zero{F}() = \Zero{F}$.
