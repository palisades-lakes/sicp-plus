...
// implements Ringlike<UnboundedNatural> 
{
  ...
  public final static int MAX_BITS = (MAX_WORDS << 5);
  ...
  public final UnboundedNatural words (final int i0,
                              final int i1) {
    assert 0<=i0;
    assert i0<i1;
  ...
  public final UnboundedNatural setWord (final int i,
                                final int w) {
    assert 0<=i : "Negative index: " + i;
    assert i < MAX_WORDS : "word index too large" + i;
    if (0==w) {
  ...
  /** Singleton. */
  public static final UnboundedNatural ZERO = new UnboundedNatural(new int[0]); 

  @Override
  public final boolean isZero () { return 0==hiInt(); }

  @Override
  public final UnboundedNatural zero () { return ZERO; }
  ...
  static final UnboundedNatural sum (final long u,
                            final long v,
                            final int upShift) {
    assert 0L<=u;
    assert 0<=upShift;
  ...
  static final UnboundedNatural difference (final long u,
                                   final long v,
                                   final int upShift) {
    assert 0L<=u;
    assert 0<=upShift;
  ...
  static final UnboundedNatural difference (final long v,
                                   final int upShift,
                                   final long u) {
    assert 0L<=u;
    assert 0<=upShift;
  ...
  static final UnboundedNatural product (final long t0,
                                final long t1) {
    assert 0L<=t0;
    assert 0L<=t1;
    ...
    if (0!=w3) { return new UnboundedNatural(new int[] {w0,w1,w2,w3,}); }
    if (0!=w2) { return new UnboundedNatural(new int[] {w0,w1,w2,}); }
    if (0!=w1) { return new UnboundedNatural(new int[] {w0,w1,}); }
    if (0!=w0) { return new UnboundedNatural(new int[] {w0,}); }
    return ZERO; }

  // TODO: fix lurking overflow issue
  // probably only works as long as t is double significand

  static final UnboundedNatural fromSquare (final long t) {
    assert 0L<=t;
    //if (0L==t) { return zero(); }
  ...
  public final UnboundedNatural add (final long u) {
    assert 0L<u;
    //if (0L==u) { return this; }
    final int nt = hiInt();
  ...
  private final UnboundedNatural addByWords (final long u,
                                    final int iShift) {
    final int nt = hiInt();
    final int[] tt = words();
  ...
  private final UnboundedNatural addByBits (final long u,
                                   final int iShift,
                                   final int bShift) {
    final int nt = hiInt();
    final int[] tt = words();
  ...
  public final UnboundedNatural subtract (final long u) {
    assert 0L<=u;
  ...
  private final UnboundedNatural subtractByWords (final long u,
                                         final int iShift) {
    final int nt = hiInt();
    final int[] tt = words();
    final int[] vv = new int[nt];
  ...
  private final UnboundedNatural subtractByBits (final long u,
                                        final int iShift,
                                        final int bShift)  {
    final int nt = hiInt();
  ...
  public final UnboundedNatural subtractFrom (final long u) {
    assert 0L<=u;
    assert 0>=compareTo(u);
    //if (0L==u) { return this; }
    // at least 1 element in tt or u==0
  ...
  private final UnboundedNatural subtractFromByWords (final long u,
                                             final int iShift) {
    final int nt = hiInt();
    final int[] tt = words();
  ...
  private final UnboundedNatural subtractFromByBits (final long u,
                                            final int iShift,
                                            final int bShift) {
    final int nt = hiInt();
    final int[] tt = words();
  ...
  private final UnboundedNatural addByWords (final UnboundedNatural u,
                                    final int iShift) {
    final int nt = hiInt();
    final int[] tt = words();
  ...
  private final UnboundedNatural addByBits (final UnboundedNatural u,
                                   final int iShift,
                                   final int bShift) {
    final int nt = hiInt();
    final int[] tt = words();
  ...
  @Override
  public final UnboundedNatural abs () { return this; }
  ...
  @Override
  public final UnboundedNatural subtract (final UnboundedNatural u) {
    assert 0<=compareTo(u);
    final int nt = hiInt();
    final int nu = u.hiInt();
    assert nu<=nt;
  ...
  @Override
  public final UnboundedNatural absDiff (final UnboundedNatural uu) {
    //    //assert isValid();
    //    //assert u.isValid();
    final UnboundedNatural u = uu;
  ...
  static final UnboundedNatural ONE = new UnboundedNatural(new int[] {1});

  @Override
  public final UnboundedNatural one () { return ONE; }

  public final static UnboundedNatural ones (final int n) {
    final int[] vv = new int[n];
    Arrays.fill(vv, -1);
    return unsafe(vv,n); }
  ...
  private final UnboundedNatural squareSimple () {
    final int nt = hiInt();
    final int[] tt = words();
  ...
  @Override
  public final UnboundedNatural square () {
  ...
  @Override
  public final UnboundedNatural multiply (final UnboundedNatural u) {
    //    //assert isValid();
    //    //assert u.isValid();
  ...
  public final UnboundedNatural multiply (final long v) {
    if (0L==v) { return ZERO; }
    if (1L==v) { return this; }
    if (isZero()) { return ZERO; }
    assert 0L < v;
    ...
    if (0!=hi) {
      ...
      for (;i<n0;i++) {
        ...
        carry = (product>>>32); }
      vv[i+1]= (int) carry; }
    return UnboundedNatural.unsafe(vv); }

  //--------------------------------------------------------------

  public final UnboundedNatural multiply (final long u,
                                 final int upShift) {
    ////assert isValid();
    assert 0L<=u;
    assert 0<=upShift;
    ...
    return multiply(UnboundedNatural.valueOf(u,upShift)); }
  ...
  // for testing
  public final List<UnboundedNatural> 
  divideAndRemainderKnuth (final UnboundedNatural u) {
    ////assert isValid();
    ////assert u.isValid();
  ...
  // for testing
  public final List<UnboundedNatural> 
  divideAndRemainderBurnikelZiegler (final UnboundedNatural u) {
    ////assert isValid();
    ////assert u.isValid();
  ...
  @Override
  public final List<UnboundedNatural> 
  divideAndRemainder (final UnboundedNatural u) {
    //assert isValid();
    //assert u.isValid();
    return NaturalDivide.divideAndRemainder(this,u); }

  @Override
  public final  UnboundedNatural divide (final UnboundedNatural u) {
    //assert isValid();
    //assert u.isValid();
    return divideAndRemainder(u).get(0); }

  @Override
  public final  UnboundedNatural remainder (final UnboundedNatural u) {
    //assert isValid();
    //assert u.isValid();
  ...
  @Override
  public final UnboundedNatural gcd (final UnboundedNatural u) { 
    //assert isValid();
    //assert u.isValid();
  ...
  @Override
  public final List<UnboundedNatural> reduce (final UnboundedNatural d) {
    //assert isValid();
  ...
  private final UnboundedNatural shiftDownByWords (final int iShift) {
    final int nt = hiInt();
  ...
  private final UnboundedNatural shiftDownByBits (final int iShift,
                                         final int bShift) {
    final int nt = hiInt();
    final int nv = nt-iShift;
  ...
  public final UnboundedNatural shiftDown (final int downShift) {
    assert 0<=downShift;
  ...
  private final UnboundedNatural shiftUpByWords (final int iShift) {
    final int nt = hiInt();
    final int nv = nt+iShift;
    final int[] tt = words();
    final int[] vv = new int[nv];
  ...
  private final UnboundedNatural shiftUpByBits (final int iShift,
                                       final int bShift) {
    final int nt = hiInt();
    final int nv = nt+iShift;
    final int rShift = 32-bShift;
    ...
    if (0!=vvn) { vv[nv] = vvn; return new UnboundedNatural(vv); }
    final int[] vvv = new int[nv];
    for  (int i=0;i<nv;i++) { vvv[i] = vv[i]; }
    return new UnboundedNatural(vvv); }

  public final UnboundedNatural shiftUp (final int upShift) {
    assert 0<=upShift;
  ...
  public final UnboundedNatural setBit (final int i) {
    assert 0<=i;
    final int iw = (i>>>5);
    final int w = word(iw);
    final int ib = (i&0x1F);
  ...
  private UnboundedNatural (final int[] words) { 
  ...
  private static final UnboundedNatural unsafe (final int[] words,
                                       final int hiInt){
    if (hiInt<words.length) {
      final int[] ww = new int[hiInt];
      for (int i=0;i<hiInt;i++) { ww[i] = words[i]; }
      return new UnboundedNatural(ww); }
    return new UnboundedNatural(words); }

  /** Doesn't copy <code>words</code>. 
   */

  static final UnboundedNatural unsafe (final int[] words) {
    final int hi = Ints.hiInt(words);
    return unsafe(words,hi); }

  /** Copy <code>words</code>. 
   *  */
  public static final UnboundedNatural make (final int[] words) {
    final int end = Ints.hiInt(words);
    return new UnboundedNatural(Arrays.copyOf(words,end)); }
